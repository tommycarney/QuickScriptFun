...
=begin rdoc
YAML[http://www.yaml.org] stands for "YAML Ain't Markup Language" and is a simple human-readable data markup format.
=end
...
class CurrencyConverter
	BASE_URL = 'http://themoneyconverter.com/rss-feed'
	CURRENCY_CODES = {
		'EUR' => 'Euro',
		'CAD' => 'Canadian Dollar', 
		'CNY' => 'Chinese Yuan', 
	...
	def get_rate_for_abbr_from_raw_rate_lines(abbr, raw_rate_lines)
		regex = {
			:open => 
				/<description>1 #{@name} = /,
			:close => 
				/ #{CURRENCY_CODES[abbr]}<\/description>/
		}
		line = raw_rate_lines.detect { |line| line =~ /#{CURRENCY_CODES[abbr]}/ }
...
=end
	...
	def get_xml_lines()
		open("#{BASE_URL}/#{@base_currency}/rss.xml").readlines.find_all do |line|
			line =~ /1 #{@name} = /
		end
	end
...
This is a side-effect akin to memoization, hence bang.
=end
